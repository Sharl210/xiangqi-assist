...
- **可选自动落子**：通过无障碍手势执行点击或拖动，并在落子前后进行局面和通道检查；点击式落子从起点到终点的间隔以 220ms 为基准逐手随机取 30%～100%，首次工作模式为自动，也可关闭自动走子开关只看建议。未点击“一键启动”时不会执行任何识别或落子。
...
| APK | 适用设备 |
|---|---|
| `xiangqi-assist-1.1-armv8.apk` | 通用 64 位 ARM Android 设备，兼容性优先 |
| `xiangqi-assist-1.1-armv8-dotprod.apk` | 支持 ARMv8.2 dotprod 指令的较新设备，优先发挥对应引擎优化 |
