 ...
 *
 * FIXTURE_ROWS 为真实 YOLOv5 象棋检测输出采样（旧轻量模型数据，仅作为解码回归夹具；
 * 主模型工件的端到端重复识别由 `tools/validate_yolo_model.py` 独立验证）
 */
class YoloPipelineTest {
    ...
    fun `ambiguous class prediction is rejected instead of guessed`() {
        ...
        val rows = Array(YoloPostprocessor.ANCHORS) { FloatArray(YoloPostprocessor.DIMS) }
        put(rows, 0, 320f, 320f, 60f, 60f, 0.9f, 4)
        rows[0][5 + 4] = 0.70f
        rows[0][5 + 5] = 0.65f
        val dets = YoloPostprocessor.decode(
            rows, lb, 640, 640, confThreshold = 0.50, classMarginMin = 0.10
        )
