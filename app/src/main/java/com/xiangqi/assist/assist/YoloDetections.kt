...
class YoloDetection(
    ...
    val w: Double,
    val h: Double,
    /** 类别候选（按模型置信度降序）；仅作诊断记录，不参与历史类别替换。 */
    val alternatives: List<Pair<Int, Double>> = emptyList(),
) {
    val isBoard: Boolean get() = labelId == LABEL_BOARD
    /** 对应 Piece 常量；board 类别和未知标签返回 -1 */
    val piece: Int get() = PIECE_CODES.getOrElse(labelId) { -1 }
